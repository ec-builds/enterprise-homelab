...
| 004 | Docker hosts | Run infrastructure containers on a dedicated Debian VM (`docker-lab`); run media separately on a dedicated media host (`media-server-lab`) | Consolidating infrastructure containers on one VM simplifies backups, recovery, and management — the environment can be snapshotted and restored as a unit. Isolating media on its own host keeps heavy media workloads off the infrastructure VM and lets each be managed and recovered independently. |
